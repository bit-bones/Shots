...
class GameEventManager {
    ...
    emit(eventType, data) {
        ...
        console.log('[GameEvents.emit] Event created:', eventType, 'isHost:', this.isHost, 'connected:', this.connected);
        if (this.isHost && this.connected) {
            this.syncEvent(event);
        } else {
            console.log('[GameEvents.emit] Not syncing - isHost:', this.isHost, 'connected:', this.connected);
        }
        return event;
    }
    ...
    syncEvent(event) {
        try {
            if (window.ws && window.ws.readyState === WebSocket.OPEN) {
                console.log('[GameEvents] Sending event to joiner:', event.type, event.data);
                window.ws.send(JSON.stringify({ type: 'relay', data: { type: 'game-event', event } }));
            } else {
                console.warn('[GameEvents] Cannot sync - WebSocket not ready. isHost:', this.isHost, 'connected:', this.connected);
            }
        } catch (e) {
    ...
    processEvent(event) {
        console.log('[GameEvents] Processing incoming event:', event.type, event.data);
        switch (event.type) {
            case 'explosion':
