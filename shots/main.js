...
window.gameWorldMaster.syncActionFromNet = function(data) {
    ...
    window.gameWorldMasterInstance.controlledEffect = data.effectName;
    window.gameWorldMasterInstance.executeControlledEffect(data.x, data.y);
};
...
function setupWorldMasterInstance(isLocal) {
    // Use global WorldMaster and WorldMasterUI (must be loaded via <script> tags)
    ...
    // Attach map click event for manual effects
    const canvas = document.getElementById('game-canvas');
    if (canvas && !canvas._wmClickAttached) {
        canvas.addEventListener('click', function(e) {
            if (window.localPlayerIndex === -1 && window.gameWorldMasterInstance) {
...
function updateWorldMasterSetupUI() {
    ...
    if (!isMultiplayer) {
        ...
        wmMpRoles.innerHTML = '';
        // Build radio buttons for each player
        ...
        // Add 'None' option if only 2 players
        if (lobbyPlayers.length === 2) {
            ...
            radio.name = 'wm-role-radio';
            radio.value = 'none';
            radio.checked = (worldMasterPlayerIndex === null);
            radio.onclick = () => {
                worldMasterPlayerIndex = null;
                updateWorldMasterSetupUI();
            };
            label.appendChild(radio);
        ...
        // Default: if 3 players, assign joiner2 as world master
    ...
    if (!isMultiplayer) {
        ...
        if (lobbyPlayers.length === 2) {
            if (worldMasterPlayerIndex === null) {
                ...
                // One is world master, other fights AI
...
function updateWorldMasterRoleBanner() {
    ...
    if (window.localPlayerIndex === -1) {
        banner.innerText = 'You are the World Master!';
...
// Match settings
let ROUNDS_TO_WIN = 3; // default, can be changed in setup UI
...
class Player {
        ...
        if (this.burning) {
            ...
            if (this.flameParticles.length < 14 && Math.random() < 0.9) {
                this.flameParticles.push({
                    x: this.x + (Math.random()-0.5) * this.radius * 1.2,
    ...
    draw(ctx) {
        ...
        if (this.burning) {
            ...
            ctx.beginPath();
            ctx.arc(this.x + shakeX, this.y + shakeY + 4, this.radius+10, 0, Math.PI*2);
        ...
        if (this.flameParticles && this.flameParticles.length) {
            for (let fp of this.flameParticles) {
                ...
                // particle core
                ctx.beginPath();
                ctx.fillStyle = `hsla(${fp.hue},100%,60%,${0.9 * t})`;
                ctx.arc(fp.x, fp.y, fp.r, 0, Math.PI*2);
                ctx.fill();
                // soft outer glow
                const grad = ctx.createRadialGradient(fp.x, fp.y, 0, fp.x, fp.y, fp.r * 4);
                grad.addColorStop(0, `hsla(${fp.hue},100%,65%,${0.9 * t})`);
                grad.addColorStop(0.5, `hsla(${fp.hue+30},100%,50%,${0.45 * t})`);
    ...
    takeDamage(dmg) {
        ...
        createSyncedDamageFlash(this, dmg, !!this.burning);
    }
...
class Obstacle {
    ...
    chipChunksAt(x, y, radius, power = 1, obliterate = false, explosion = false) {
        ...
        for (let i = 0; i < hits.length; ++i) {
            ...
            if (c.hp <= 0) {
                ...
                try {
                    ...
                    updates.push({ i: ci, destroyed: !!c.destroyed, flying: !!c.flying, vx: c.vx||0, vy: c.vy||0, alpha: c.alpha||1, x: c.x, y: c.y });
                } catch (e) {}
            } else {
        ...
        // If host in multiplayer, send minimal visual-only chunk updates so joiners can mirror visuals
        if (updates.length > 0) {
            const obsIdx = obstacles.indexOf(this);
...
function syncToJoiner(eventType, data) {
    try {
        if (NET && NET.role === 'host' && NET.connected && typeof GameEvents !== 'undefined') {
            console.log('[SyncToJoiner] Emitting event:', eventType, data);
            GameEvents.emit(eventType, data);
        } else {
            console.log('[SyncToJoiner] Not syncing - Role:', NET?.role, 'Connected:', NET?.connected, 'GameEvents available:', typeof GameEvents !== 'undefined');
        }
    } catch (e) { 
    ...
    if (!waitingForCard && (!NET.connected || NET.role === 'host')) {
        ...
        for (let p of participants) {
            if (p.health <= 0) {
                ...
                try {
                    if (typeof ROUNDS_TO_WIN !== 'number' || ROUNDS_TO_WIN <= 0) ROUNDS_TO_WIN = 3;
                ...
                const powerupChoices = randomChoice(POWERUPS, 5).map(c => c.name);
                ...
                if (roundsSinceLastModifier >= worldModifierRoundInterval) {
                    ...
                    const modPool = WORLD_MODIFIERS;
                    const modChoices = randomChoice(modPool, 3).map(c => c.name);
...
function draw() {
    ...
    const eName = (enemy && (enemy.displayName)) || (NET.peerName || 'Shot bot');
        ...
        // Multiplayer: map host/joiner to left/right depending on NET.role
        const isJoiner = (NET.role === 'joiner');
        const hostEntity = isJoiner ? enemy : player;
        const joinerEntity = isJoiner ? player : enemy;
        const hostName = (NET.role === 'host') ? (NET.myName || (player && player.displayName) || 'Player 1') : (NET.peerName || (hostEntity && hostEntity.displayName) || 'Player 1');
        const joinerName = (NET.role === 'host') ? (NET.peerName || (joinerEntity && joinerEntity.displayName) || 'Player 2') : (NET.myName || (joinerEntity && joinerEntity.displayName) || 'Player 2');
        ctx.fillStyle = "#65c6ff";
        ctx.fillText(hostName + ": " + ((hostEntity && typeof hostEntity.score === 'number') ? hostEntity.score : '0'), 24, 34);
        ctx.fillStyle = "#ff5a5a";
        if (joinerEntity) ctx.fillText(joinerName + ": " + ((joinerEntity && typeof joinerEntity.score === 'number') ? joinerEntity.score : '0'), CANVAS_W - 220, 34);
    }
    ctx.restore();
...
function drawPlayer(p) {
    ...
    // Always keep the player's base color. For damage flashes we apply
    // a shadow + a translucent overlay so the underlying color remains
    // visible instead of swapping to a different fill color.
    let baseColor = p.color;
    let flashOverlay = null;
    if (p.damageFlash > 0) {
        let t = Math.min(1, p.damageFlash / 0.25);
        if (p.burning) {
            // orange-ish burn highlight (translucent)
            ctx.shadowColor = "rgba(255,130,40,0.95)";
            ctx.shadowBlur = 30 * t;
            flashOverlay = `rgba(255,140,40,${0.18 * t})`;
        } else {
            // white hit flash (translucent)
            ctx.shadowColor = "#fff";
            ctx.shadowBlur = 30 * t;
            flashOverlay = `rgba(255,255,255,${0.48 * t})`;
        }
    } else {
    ...
    // Draw base circle (player's normal color)
    ctx.beginPath();
    ...
    // Draw a translucent flash overlay on top if damaged, preserving the base color
    if (flashOverlay) {
        ctx.save();
        ctx.beginPath();
        ctx.arc(p.x + shakeX, p.y + shakeY, p.radius, 0, Math.PI*2);
        ...
        ctx.globalAlpha = 1;
        // no additional shadow for overlay; the main shadow was applied to base fill
        ctx.shadowBlur = 0;
        ctx.fill();
        ctx.restore();
    }
...
function netShowPowerupCards(choiceNames, chooserRole) {
    ...
    const loser = getEntityForRole(chooserRole); // chooser is the loser of the round
            ...
            for (let i = 0; i < cardCount; ++i) {
                ...
                card.onclick = () => {
                    ...
                    if (hadPending) {
                        ...
                        setTimeout(() => netShowWorldModifierCards(offer.choices, offer.chooserRole, offer.finalIdx), 700);
                        try { if (window.ws && window.ws.readyState === WebSocket.OPEN) window.ws.send(JSON.stringify({ type:'relay', data:{ type:'mod-offer', choices: offer.choices, chooserRole: offer.chooserRole, finalIdx: offer.finalIdx } })); } catch (e) {}
                    }
                };
                div.appendChild(card);
            }
...
function netShowWorldModifierCards(choiceNames, chooserRole, finalIdx /* optional */) {
    if (matchOver) return; // suppress during match-end modal
    ...
    // neutral AI selection: host decides the final pick and broadcasts mod-apply; clients only display
    const hostWillDecide = (!NET.connected) || (NET.connected && NET.role === 'host');
    // If caller provided finalIdx (from host), honor it; otherwise pick locally
    if (typeof finalIdx !== 'number' || finalIdx < 0 || finalIdx >= cardCount) {
        finalIdx = randInt(0, Math.max(0, cardCount-1));
    }
    function cycleOnce() {
        clearAll();
        let hi;
        ...
        if (cycle < 4) {
            ...
            // final highlight
            clearAll();
            highlightCard(finalIdx, accentPurple);
            setTimeout(() => {
                const opt = choices[finalIdx];
                ...
                try { const card = div.childNodes[finalIdx]; if (card && card._accentStyle) { card._accentStyle.remove(); card._accentStyle = null; } } catch (e) {}
                div.style.display = 'none'; div.innerHTML = ''; div.classList.remove('card-bg-visible');
...
function updateCardsUI() {
    ...
    if (!NET.connected) {
        // Single player: use local name and fallback
    ...
    function buildHtmlForWorldList(wlist) {
        ...
        return wlist.map(n => `<span class="card-badge" title="World modifier: ${n}">${n}</span>`).join('');
    }

    cardsDiv.innerHTML =
        `<div class="cards-list"><span style="color:#65c6ff;font-weight:bold;">${hostName} Cards:</span> ${buildHtmlForList(p1Cards)}</div>` +
        `<div class="cards-list" style="margin-top:7px;"><span style="color:#ff5a5a;font-weight:bold;">${joinerName} Cards:</span> ${buildHtmlForList(p2Cards)}</div>` +
    `<div class="cards-list" style="margin-top:7px;"><span style="color:#8f4f8f;font-weight:bold;">World Cards:</span> ${buildHtmlForWorldList(worldList)}</div>`;
}
...
function setupOverlayInit() {
        ...
        window.ws.onmessage = function(event) {
            ...
            if (msg.type === 'hosted') {
                ...
                if (data && data.type === 'game-event' && data.event) {
                    console.log('[WS] Received game-event, processing:', data.event.type);
                ...
                if (data && data.type === 'input' && NET.role === 'host') {
                    ...
                    if (!matchOver) {
                        ...
                        setTimeout(() => netShowWorldModifierCards(data.choices||[], data.chooserRole, finalIdx), 200);
                    }
            ...
            if (pending.kind === 'card') {
                ...
                try {
                    if (window._pendingWorldModOffer) {
                        ...
                        window._pendingWorldModOffer = null;
                        try { waitingForCard = true; } catch (e) {}
                        try { netShowWorldModifierCards(offer.choices, offer.chooserRole, offer.finalIdx); } catch (e) {}
                        try { if (window.ws && window.ws.readyState === WebSocket.OPEN) window.ws.send(JSON.stringify({ type:'relay', data:{ type:'mod-offer', choices: offer.choices, chooserRole: offer.chooserRole, finalIdx: offer.finalIdx } })); } catch (e) {}
                    }
                } catch (e) {}
    ...
    document.getElementById('start-btn').onclick = () => {
        ...
        try {
            ...
            if (typeof v === 'number' && !isNaN(v) && v > 0) ROUNDS_TO_WIN = v; else ROUNDS_TO_WIN = 3;
        } catch (e) { ROUNDS_TO_WIN = 3; }
        try { localStorage.setItem('shape_shot_rounds', String(ROUNDS_TO_WIN)); } catch (e) {}
