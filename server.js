        ...
        if (msg.type === 'host') {
            ...
            session.joiners.forEach((entry, idx) => {
                if (entry && entry.ws && entry.ws.readyState === WebSocket.OPEN) {
                    safeSend(ws, { type: 'peer-joined', joinerIndex: idx });
                }
            });
            ...
            session.joiners[slot] = { ws, index: slot };
            ws.role = 'joiner';
            ws.sessionCode = session.code;
            ws.joinerIndex = slot;
            safeSend(ws, { type: 'joined', code: session.code, joinerIndex: slot, maxPlayers: MAX_JOINERS + 1 });
            safeSend(session.host, { type: 'peer-joined', joinerIndex: slot });
            // Let other joiners know someone new arrived
            broadcastToJoiners(session, { type: 'peer-joined', joinerIndex: slot }, ws);
